    ...
    const getPlanet = (rule: keyof typeof constant.Dignity) => {
        ...
        return (name: keyof typeof dignity) => {
            ...
            const checkRange = (range: number[][]) => {
                return range.map(([start, end]: number[]) => ((posPlanet.position.longitude.absolute % 360) >= start && end >= (posPlanet.position.longitude.absolute % 360))).filter((value) => value).length > 0;
            }
            ...
            switch (rule) {
                case 'TRIPLICITY': {
                    ...
                    .keys(range)
                    .map((key) => {
                        const [first, second, partner] = (<number[][][]>(range[<keyof typeof range>key][isDiurnal ? 0 : 1]))
                ...
                case 'TERM': {
                    ...
                    .keys(range)
                    .map((key) => {
                        result[util.convertUpperCaseToCapitalize(key)] = checkRange(range[<keyof typeof range>key])
                    });
