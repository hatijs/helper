...
export const position = (tjdUT: number, geoLon: number, geoLat: number) => {
    ...
    const getLot = (name: keyof typeof constant.Lot) => {
        ...
        const basis = () => {
            ...
            const love = asc.ascendant + (isDiurnal ? lotOfSpirit - lotOfFortune : lotOfFortune - lotOfSpirit);
            const necessity = asc.ascendant + (isDiurnal ? lotOfFortune - lotOfSpirit : lotOfSpirit - lotOfFortune);

            return util.isDiurnal(tjdUT, geoLon, geoLat, love) ? necessity : love;
        }
        ...
        const exaltation = () => {
            ...
            return asc.ascendant + (isDiurnal ? 19 - sun.position.longitude.absolute : 33 - moon.position.longitude.absolute);
        }
        ...
        const fortune = (inversion: boolean = false) => {
            ...
            return asc.ascendant + ((isDiurnal && !inversion || !isDiurnal && inversion) ? moon.position.longitude.absolute - sun.position.longitude.absolute : sun.position.longitude.absolute - moon.position.longitude.absolute);
        }
